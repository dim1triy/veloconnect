...
class SearchResultResponse extends AbstractResponse
{
    ...
    public function receive($data)
    {
        ...
        // get ItemDetail
        $items = $xml->getElementsByTagNameNS('*','ItemDetail');
        // 
        $this->items = array();
        // 
        if ($items->length)
        {
            ...
            foreach ($items as $item)
            {
                ...
                // 
                $itemObj->Availability = is_object($obj->item(0)) ? trim($obj->item(0)->nodeValue) : false;
            }
        }
    }
     ...
     * _getItemInformation
     * 
     * @param \DOMNodeList $information
     * @param \Miechuliv\Veloconnect\ComplexTypes\Item $item
     */
